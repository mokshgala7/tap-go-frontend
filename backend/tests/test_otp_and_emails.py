...
def run_all_tests():
    print("==================================================")
    print("TAP & GO REAL EMAIL & OTP SYSTEM COMPREHENSIVE TESTS")
    print("==================================================")

    # ----------------------------------------------------
    # TEST 1: Direct Gmail SMTP Delivery to Support Email
    # ----------------------------------------------------
    print("\n[TEST 1] Testing Real Gmail SMTP Delivery...")
    test_otp = "782941"
    smtp_success = send_registration_otp(
        to_email="tapandgosupport@gmail.com",
        otp=test_otp,
        account_type="passenger",
    )
    print(f"  Gmail SMTP direct send result: {smtp_success}")
    assert smtp_success is True, "Gmail SMTP failed to send registration OTP email!"
    ...
    with SessionLocal() as db:
        ...
        assert log_entry.status == "SENT", f"Expected status SENT, got {log_entry.status}"
        print("  ✓ Email delivery logged to database with status SENT.")
    ...
    with SessionLocal() as db:
        ...
        # login returns JSONResponse with success=False
    ...
    with SessionLocal() as db:
        ...
        # 4b. Critical Resiliency Test: SMTP Failure MUST NOT Rollback Financial Transaction
        print("  Simulating complete SMTP network failure during ride payment...")
        with patch("app.utils.email_service._send_smtp_email", side_effect=Exception("SMTP Connection Timeout")):
            ...
            print("  ✓ CRITICAL VERIFICATION: Financial transaction succeeded despite SMTP failure! Zero rollback!")
        ...
        with patch("app.utils.email_service._send_smtp_email", side_effect=Exception("SMTP Down")):
            ...
            print("  ✓ Withdrawal completed and committed despite SMTP failure!")
